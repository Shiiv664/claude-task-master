...
/**
 * Safe logger wrapper that suppresses output during MCP server context
 */
function safeLogger(logger) {
	if (!logger) return { info: () => {}, warn: () => {} };
	
	return {
		info: (message) => {
			if (!isMcpContext() && logger.info) {
				logger.info(message);
			}
		},
		warn: (message) => {
			if (!isMcpContext() && logger.warn) {
				logger.warn(message);
			}
		}
	};
...
export function findTasksPath(explicitPath = null, args = null, log = null) {
	const logger = safeLogger(log || console);
